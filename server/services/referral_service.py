...
この時期に成立させないと、100人を集めたいまさにその期間に招待報酬が1件も出ず、
集客装置が集客したい時期に止まってしまう。

交換が開いた後は **招待された人が電話番号を登録した時点**に切り替える。
登録だけで報酬が出続けると、「登録して使わない人」を招待し続ける動機が残るため。
...
def settles_on_registration(session: Session) -> bool:
    """いま「登録した時点」で成立させる時期かどうか。

    事前登録の期間＝交換がまだ開いていない期間、と定義している。
    別の設定値を増やさないのは、この2つが実務上いつも同じ日で動くため
    """
    return not campaign_service.withdrawals_open(session)


def is_settled_condition_met(session: Session, invitee: User) -> bool:
    if settles_on_registration(session):
        return True
    # 交換が開いた後は電話番号の登録をもって成立とする。
    # TODO: アンケート機能（別タスク）が入ったら回答済みも条件に加える
